 ...
 * Нет: станций, мирных кораблей, газовых гигантов — жить и торговать тут негде.
 * Планеты есть, но всегда мёртвые: generatePlanet при чёрной дыре принудительно
 * ставит isEmpty, так что колоний и контрактов у них не бывает — только разведка.
 * Боссы добавляются отдельно в постобработке generateGalaxy.
 * Сумма долей — ровно 1.0, остаток по правилам getLocationType ушёл бы в аномалии.
 */
export const BLACK_HOLE_LOCATION_CHANCES = {
    anomaly: 0.28,       // аномальная физика рядом с сингулярностью
    enemy: 0.17,         // пираты используют ЧД как убежище
    planet: 0.14,        // мёртвые миры, с которых сингулярность сорвала всё живое
    storm: 0.13,         // гравитационные и радиационные шторма
    asteroidBelt: 0.10,  // осколки разорванных приливными силами планет
    derelictShip: 0.09,  // корабли, не переживших притяжение
    wreckField: 0.06,    // поля обломков затянутых гравитацией
    distressSignal: 0.03,// кто-то всё ещё застрял
};
