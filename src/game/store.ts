...
import { createLogSlice, createShipSlice } from "@/game/slices";
import type {
    BattleResult,
    CargoItem,
    CrewMember,
...
export const useGameStore = create<GameStore>()(
    immer((set, get) => ({
        ...
        getTotalConsumption: () => {
            const state = get();
            const pilotRed = state.crew.find(
                (c) => c.assignment === "navigation",
            )
                ? 1
                : 0;
        ...
        nextTurn: () => {
            ...
            if (crewInDamagedModules.length > 0) {
                crewInDamagedModules.forEach((crewMember) => {
                    ...
                    const healthPercent =
                        (shipModule.health / (shipModule.maxHealth || 100)) *
                        100;
            ...
            if (powerDeficit > 0) {
                ...
                const updatedModules = [
                    ...currentStatePowerDeficit.ship.modules,
                ];
            ...
            get().crew.forEach((c) => {
                    ...
                    switch (c.assignment) {
                        case "repair": {
                            ...
                            if (crewRace?.id === "xenosymbiont") {
                                const symbiosisTrait =
                                    crewRace.specialTraits.find(
                                        (t) => t.effects.canMerge,
                                    );
                                if (symbiosisTrait) {
                                    repairAmount = Math.floor(
                ...
                c.traits?.forEach((trait) => {
                    ...
                    if (
                        trait.effect.teamMorale &&
                        crewInSameModule.length > 0
                    ) {
                    ...
                    if (
                        trait.effect.moralePenalty &&
                        crewInSameModule.length > 0
                    ) {
                        const moralePenalty = Math.abs(
                            trait.effect.moralePenalty,
                        );
                        set((s) => ({
                            crew: s.crew.map((cr) =>
                ...
                if (crewRace?.specialTraits) {
                    ...
                    if (
                        penaltyTrait &&
                        penaltyTrait.effects.alienPresencePenalty
                    ) {
            ...
            const boost = get().crew.find((c) => c.assignment === "power")
                ? 5
                : 0;
            ...
            if (available < 0) {
                ...
                if (Math.random() < 0.4) {
                    const mod =
                        get().ship.modules[
                            Math.floor(
                                Math.random() * get().ship.modules.length,
                            )
                        ];
                    set((s) => ({
                        ship: {
                    ...
                    get().addLog(
                        `"${mod.name}" повреждён перегрузкой!`,
                        "error",
                    );
                }
            }
            ...
            activeCursedArtifacts.forEach((artifact) => {
                ...
                switch (artifact.negativeEffect.type) {
                    ...
                    case "crew_desertion": {
                        ...
                        const chance =
                            (artifact.negativeEffect.value || 5) / 100;
            ...
            if (state.currentCombat) {
                ...
                if (eDmg > 0) {
                    ...
                    if (tgt) {
                        ...
                        // Check for dead crew
                        const deadCrew = get().crew.filter(
                            (c) => c.health <= 0,
                        );
                        if (deadCrew.length > 0) {
                            set((s) => ({
        ...
        selectSector: (sectorId) => {
            ...
            if (warpCoil) {
                ...
                get().addLog(
                    `⚠ Пилот не в кабине! Расход топлива +50%`,
                    "warning",
                );
            }
            ...
            if (distance === 0) {
                ...
                if (travelInstant) {
                    ...
                    get().addLog(
                        `⚡ Мгновенный перелёт в ${sector.name}!`,
                        "info",
                    );
                } else {
                    get().addLog(
        ...
        startCombat: (enemy, isAmbush = false) => {
            ...
            if (isAmbush) {
                get().addLog(
                    `⚠️ ЗАСАДА! ${enemy.name} атакует первым!`,
                    "error",
                );
        ...
        startBossCombat: (bossLocation) => {
            ...
            const bossModules: EnemyModule[] = bossData.modules.map(
                (m, idx) => ({
                    id: idx,
                    type: m.type,
                    name: m.name,
            ...
            get().addLog(
                `Регенерация: ${bossData.regenRate}% за ход`,
                "warning",
            );
        },
        ...
        attackEnemy: () => {
            ...
            if (aiCoreModules > 0) {
                ...
                get().addLog(
                    `🤖 ИИ Ядро: +${aiCoreModules * 5}% точность`,
                    "info",
                );
            }
            ...
            if (weaponCounts.missile > 0) {
                ...
                if (missileInterceptedCount > 0) {
                    logs.push(
                        `🛡️ ${missileInterceptedCount} ракета(ы) сбита(ы)!`,
                    );
                }
            }
            ...
            if (missedShots.missile > 0) {
                logs.push(
                    `❌ ${missedShots.missile} ракета(ы) промахнул(ись)!`,
                );
            }
            ...
            if (totalShieldDamage > 0) {
                ...
                get().addLog(
                    `Урон щитам врага: ${totalShieldDamage}`,
                    "combat",
                );
            }
            ...
            if (totalModuleDamage > 0 || weaponCounts.kinetic > 0) {
                ...
                const finalDamage = Math.max(
                    1,
                    totalModuleDamage - moduleDefense,
                );
            ...
            if (
                ...
                const blackBox = state.artifacts.find(
                    (a) =>
                        a.effect.type === "credit_booster" && a.effect.active,
                );
                if (blackBox) {
                    const boostValue =
                        getArtifactEffectValue(blackBox, state) / 100;
                    creditsAmount = Math.floor(
                        creditsAmount * (1 + boostValue),
                    );
                }
                ...
                if (updatedCombat.enemy.isBoss && loot.guaranteedArtifact) {
                    ...
                    if (artifact && !artifact.discovered) {
                        ...
                        if (miningContract) {
                            set((s) => ({
                                credits:
                                    s.credits + (miningContract.reward || 0),
                            }));
                            get().addLog(
            ...
            const pilotInCockpit =
                pilot &&
                state.ship.modules.find((m) => m.id === pilot.moduleId)
                    ?.type === "cockpit";
            ...
            if (
                ...
                get().addLog(
                    `✈️ Пилот ${pilot.name} уклонился от атаки!`,
                    "info",
                );
            ...
            // ═══════════════════════════════════════════════════════════════
            const krylorianCrew = state.crew.filter(
                (c) => c.race === "krylorian",
            );
            let evasionChance = 0;
            ...
            if (evasionChance > 0 && Math.random() < evasionChance) {
                ...
                if (activeMods.length > 0) {
                    ...
                    const sortedMods = [...activeMods].sort(
                        (a, b) =>
                            getModuleTargetPriority(b) -
                            getModuleTargetPriority(a),
                    );
                    tgt = sortedMods[0];
                }
                ...
                const damageCrewInModule = (
                    ...
                    crewInModule.forEach((c) => {
                        const newHealth =
                            get().crew.find((cr) => cr.id === c.id)?.health ||
                            0;
                        if (newHealth <= 0) {
                    ...
                    crewInDamagedModule.forEach((c) => {
                        c.traits?.forEach((trait) => {
                            if (trait.effect.combatMoraleDrain) {
                                const moraleDrain =
                                    trait.effect.combatMoraleDrain;
                                set((s) => ({
                                    crew: s.crew.map((cr) =>
                                        cr.id === c.id
                                            ? {
            ...
            if (currentCombat?.enemy.isBoss) {
                ...
                if (boss.regenRate && boss.regenRate > 0) {
                    const aliveModules = boss.modules.filter(
                        (m) => m.health > 0,
                    );
                    if (aliveModules.length > 0) {
