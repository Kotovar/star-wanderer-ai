...
export function StormPanel() {
  ...
  const recentStormLogs = log.slice(0, 10).filter(entry =>
  ...
  if (stormCompleted) {
    ...
    return (
      <div className="bg-[rgba(50,0,50,0.3)] border-2 p-4" style={{ borderColor: info.color }}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold font-['Orbitron']" style={{ color: info.color }}>
            {info.icon} {info.name}
          </h2>
          <span className="text-sm text-[#00ff41]">✓ Исследовано</span>
        </div>

        <div className="bg-[rgba(0,0,0,0.4)] p-3 mb-4 border" style={{ borderColor: info.color }}>
        ...
        <div className="bg-[rgba(0,0,0,0.3)] p-3 mb-4 border border-[#00ff41]">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div>
              ...
              <span className={`${ship.shields > 0 ? 'text-[#00ff41]' : 'text-[#ff4444]'} ml-1`}>{Math.floor(ship.shields)}/{ship.maxShields}</span>
            </div>
            <div>
              <span className="text-[#ffb000]">🔧 Броня:</span>
              <span className={`${ship.armor > 50 ? 'text-[#00ff41]' : 'text-[#ffb000]'} ml-1`}>{ship.armor}%</span>
            </div>
            <div>
              <span className="text-[#ff4444]">❤ Экипаж:</span>
              <span className={`${crew.filter(c => c.health > 50).length === crew.length ? 'text-[#00ff41]' : 'text-[#ffb000]'} ml-1`}>{crew.filter(c => c.health > 50).length}/{crew.length}</span>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="bg-[rgba(50,0,50,0.3)] border-2 border-[#666] p-4">
        ...
        <div className="bg-[rgba(0,0,0,0.4)] p-3 mb-4 border border-[#666]">
          <p className="text-[#888] mb-2">Датчики не могут определить тип этого объекта.</p>
          <p className="text-[#ffb000]">Требуется сканер для получения информации.</p>
        </div>

        <div className="bg-[rgba(0,0,0,0.3)] p-3 mb-4 border border-[#ff4444]">
          <p className="text-[#ff4444] font-bold mb-2">⚠ ПРЕДУПРЕЖДЕНИЕ</p>
        ...
        <div className="bg-[rgba(0,0,0,0.3)] p-3 mb-4 border border-[#00ff41]">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div>
              <span className="text-[#00d4ff]">🛡 Щиты:</span>
              <span className="text-[#00ff41] ml-1">{ship.shields}/{ship.maxShields}</span>
            </div>
            <div>
              <span className="text-[#ffb000]">🔧 Броня:</span>
              <span className="text-[#00ff41] ml-1">{ship.armor}%</span>
            </div>
            <div>
              <span className="text-[#ff4444]">❤ Экипаж:</span>
              <span className="text-[#00ff41] ml-1">{crew.filter(c => c.health > 50).length}/{crew.length}</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-[rgba(50,0,50,0.3)] border-2 p-4" style={{ borderColor: info.color }}>
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-sm text-[#ffb000]">Интенсивность: {intensity} ({intensityLabels[intensity]})</span>
      </div>
