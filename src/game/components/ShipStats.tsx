...
export function ShipStats() {
  ...
  const captain = useGameStore(s => s.crew.find(c => c.profession === 'pilot'));
  ...
  return (
    <div className="bg-[rgba(0,255,65,0.05)] border border-[#00ff41] p-4 mt-2.5">
      ...
      <div className="flex justify-between mb-2 text-sm">
        ...
        <span className="text-[#9933ff]">{ship.fuel}/{ship.maxFuel}</span>
      </div>
      <div className="flex justify-between mb-2 text-sm">
      ...
      <div className="flex justify-between mb-2 text-sm">
        ...
        <span className={crew.length <= crewCapacity ? 'text-[#00ff41]' : 'text-[#ff0040]'}>
      ...
      <div className="mt-2.5 pt-2.5 border-t border-[#00ff41]">
        <div className="flex justify-between mb-2 text-sm">
          ...
          <span className="text-[#00ff41]">
            {totalPower + engineerBoost}{engineerBoost > 0 ? ` (+${engineerBoost})` : ''}
          </span>
        </div>
        <div className="flex justify-between mb-2 text-sm">
          <span className="text-[#ffb000]">⚡ Потребление:</span>
          <span className={available >= 0 ? 'text-[#00ff41]' : 'text-[#ff0040]'}>
            {totalConsumption}
          </span>
        </div>
      ...
      <div className="flex justify-between mb-2 text-sm">
        ...
        <span className="text-[#00d4ff]">{ship.shields}/{ship.maxShields}</span>
      </div>
      <div className="flex justify-between mb-2 text-sm">
        <span className="text-[#ffb000]">🔧 Броня:</span>
        <span className={ship.armor > 50 ? 'text-[#00ff41]' : 'text-[#ff0040]'}>{ship.armor}%</span>
      </div>
      <div className="flex justify-between text-sm">
        <span className="text-[#ffb000]">💨 Кислород:</span>
        <span className={crew.length <= ship.crewCapacity ? 'text-[#00ff41]' : 'text-[#ff0040]'}>
