...
const RARITY_COLORS: Record<string, { border: string; bg: string; text: string }> = {
  ...
  legendary: { border: '#ffaa00', bg: 'rgba(255, 170, 0, 0.1)', text: '#ffaa00' },
  mythic: { border: '#ff00ff', bg: 'rgba(255, 0, 255, 0.1)', text: '#ff00ff' },
...
function ArtifactCard({ artifact, onResearch, onToggle }: { 
  artifact: Artifact; 
  onResearch: () => void;
  onToggle: () => void;
}) {
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        backgroundColor: artifact.effect.active ? colors.bg : 'transparent',
      }}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="text-2xl">{artifact.discovered ? icon : '❓'}</span>
          <div>
            <div className="font-bold text-sm" style={{ color: artifact.discovered ? '#fff' : '#666' }}>
      ...
      {artifact.discovered && (
        <>
          ...
          <div className="text-xs mt-2">
            ...
            <span className={artifact.researched ? 'text-[#00ff41]' : 'text-[#ffb000]'}>
          ...
          {!artifact.researched && (
            ...
            >
              {artifact.cursed ? '⚠️ ИЗУЧИТЬ (ОПАСНО)' : 'ИЗУЧИТЬ'}
            </Button>
          )}
          ...
          {artifact.researched && (
            ...
            >
              {artifact.effect.active 
                ? artifact.cursed ? '✓ АКТИВ (НЕЙТРАЛИЗОВАТЬ)' : 'ДЕАКТИВИРОВАТЬ' 
                : 'АКТИВИРОВАТЬ'}
            </Button>
          )}
        </>
...
export function ArtifactPanel() {
  ...
  const cursedActive = artifacts.filter(a => a.cursed && a.effect.active).length;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-sm text-[#888]">
        Обнаружено: <span className="text-[#00ff41]">{discoveredCount}</span> / {artifacts.length}
        {' | '}
        Изучено: <span className="text-[#ffb000]">{researchedCount}</span>
        {' | '}
        Активно: <span className="text-[#00d4ff]">{activeCount}</span>
        {cursedActive > 0 && (
          <span className="text-[#ff0040]"> ({cursedActive} проклятых)</span>
        )}
      </div>
      
      <div className="text-xs text-[#888] mb-2">
        Учёные на борту: {scientists.length > 0 ? `Ур. ${maxScientistLevel}` : 'нет'}
      </div>
      ...
      {cursedArtifacts.length > 0 && (
        <>
          ...
          <div className="text-xs text-[#ff6666] bg-[rgba(255,0,64,0.1)] p-2 border-l-2 border-[#ff0040]">
            Сила этих артефактов сопровождается ценой. Каждый ход активный проклятый артефакт 
            оказывает негативное воздействие на корабль или экипаж.
          </div>
          <div className="grid gap-3 max-h-60 overflow-y-auto pr-2">
      ...
      <div className="bg-[rgba(255,176,0,0.1)] border border-[#ffb000] p-3 text-xs mt-2">
        ...
        <span className="text-[#888]">
          ...
          Проклятые артефакты встречаются реже, но их эффекты очень мощные — и опасные.
        </span>
      </div>
