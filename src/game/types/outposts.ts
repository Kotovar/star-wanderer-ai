...
export interface Outpost {
    ...
    /**
     * Товар и газ, оставленные игроком на складе. Отдельно от бункера, потому
     * что «забрать добычу» гребёт бункер целиком: сложенное на хранение
     * уезжало обратно в трюм тем же нажатием, ради которого его и выгружали.
     */
    storedGoods?: Partial<Record<OutpostResource, number>>;
    ...
    /**
     * Идут работы: до этого хода постройка не добывает и не обслуживает.
     * Поле снимается тиком по достижении срока — как и с захватом, потребителям
     * достаточно проверить наличие поля, а не сравнивать ходы у себя.
     */
    readyAtTurn?: number;
    /**
     * Заказанный поселенец: едет с ближайшей населённой планеты и встаёт в
     * гарнизон по приезде. Пока едет — второго не заказать: дорога и есть
     * кулдаун найма.
     */
    pendingSettler?: {
        profession: string;
        race?: RaceId;
        arrivesAtTurn: number;
    };
}
