...
export const BASE_SERVICE_VALUES = {
    ...
    /**
     * Казарма: базовая цена вербовки на ближайшей населённой планете.
     *
     * Люди не вырастают на базе из воздуха: их вербуют там, где они живут, и
     * везут. Поэтому цена и срок зависят от того, насколько глухое место вы
     * выбрали под базу, — это и есть скрытая цена глубокого сектора.
     */
    settlerCost: 1400,
    /** Надбавка к цене за каждый прыжок до планеты-донора */
    settlerCostPerHop: 0.35,
    /** Ходов на дорогу: постоянная часть и надбавка за прыжок */
    settlerTurnsBase: 3,
    settlerTurnsPerHop: 2,
...
/**
 * Сколько ходов идут работы.
 *
 * Стройка мгновенной быть не может: иначе база возникает в тот же ход, когда
 * на неё нашлись деньги, и решение «строить сейчас или подкопить» ничего не
 * стоит. Пока работы идут, постройка не добывает и не обслуживает — цена
 * измеряется не кредитами, а тем, чего вы за это время не получили. Числа
 * меньше льготного периода рейдов (15): стройплощадку не захватывают.
 */
export const BASE_BUILD_TURNS = 8;
export const BASE_UPGRADE_TURNS = 6;
export const BASE_MODULE_BUILD_TURNS = 4;
