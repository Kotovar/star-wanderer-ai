...
/**
 * Сколько топлива даёт единица дейтерия.
 *
 * 10 при цене заправки в 3₢ за единицу означает 30₢ за единицу дейтерия
 * против 17₢ при продаже: возить его в бак выгоднее, чем продавать, — иначе
 * заправка своим газом была бы красивой кнопкой, которой никто не пользуется.
 */
export const DEUTERIUM_FUEL_PER_UNIT = 10;
...
/** Ходов на сборку сборщика: до конца работ он не качает газ */
export const GAS_COLLECTOR_BUILD_TURNS = 4;
...
/**
 * Курс покупки газа у станции. Вдвое дороже базовой цены намеренно: своё
 * производство обязано оставаться выгоднее, станция лишь страхует от тупика,
 * когда рецепт требует полимеров, а метанового гиганта под сборщик не нашлось.
 */
export const GAS_BUY_RATE = 2;
