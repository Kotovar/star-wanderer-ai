...
graph TD
    ...
    subgraph Мир
        ...
        STAR[Звёзды/опасности<br/>starHazards.ts, starEffects.ts]
        STATION[Станции]
        EXPED[Экспедиции]
    ...
    STAR -->|пассивные эффекты: экипаж, добыча, сканер, энергия, бой| CREW
```
...
- **Отношения рас → экипаж**: `relations` также запускают случайные события конфликта/сближения экипажа между членами враждующих/дружественных рас (`crew/relationEvents.ts`, `random_events.logs.crew_relation_*`) — не только репутацию.
...
- **Проклятые артефакты → только живой экипаж на борту**: потурновые проклятия (мораль, мутации, дезертирство) применяются через `getLivingShipCrew` — мёртвые и приписанные к аванпостам не задеты. Урон модулям от проклятий и авторемонт от артефактов считаются в процентах от `maxHealth` (`artifacts/moduleHealth.ts`), а не плоскими единицами.
...
- **Тип звезды → пассивные эффекты**: каждый тип звезды даёт один небольшой пассивный эффект (экипаж, добыча, сканер, энергия или бой), пока корабль в секторе или летит в него как в пункт назначения (`constants/starEffects.ts`); нейтронная звезда и чёрная дыра сохраняют собственные уникальные механики.
- **Сканер/путешествие → Навигатор → GalaxyMap/SectorMap**: вход в сектор, сканирование и посещение сохраняют известные цели; навигатор ставит только визуальные отметки на обеих картах без автопрокладки маршрута или расхода топлива.
